            ...
            <div class="main-content">
                <div class="row small-spacing">
                    <div class="col-xs-12">
					  ...
					  <div id="success" class="alert alert-success" role="alert"  style="display:none"> <strong>Done !</strong>  Plan Updated Succesfully. </div>
					  <div id="delsuccess" class="alert alert-success" role="alert"  style="display:none"> <strong>Done !</strong>  Plan Deleted Succesfully. </div>
                        ...
                        <div class="box-content card white">
							...
							<div class="dropdown js__drop_down">
							 <button type="type" id="btnmultidelete"  class="btn btn-icon btn-icon-right btn-danger btn-xs waves-effect waves-light" style="margin-left:10px;"><i class="ico fa fa-trash"></i> DELETE</button>
                                <a href="add_plan.php" class="btn btn-icon btn-icon-right btn-violet btn-xs waves-effect waves-light"><i class="ico fa fa-plus"></i> Add New</a>
                            </div>
...
function fill_data(){
	...
	$.ajax({
              ...
              success: function(data) {
					...
					$.each(obj, function() {
					     result += "<tr><td><input type='checkbox'  class='checkboxtable'  name='check[]' value='"+this['plantId']+"' ></td>";
						  var photo="";
						 if(this['plantImage']==null || this['plantImage']=='')
						 { photo="noimg.png"; } else { photo = this['plantImage'];}
						 result += "<td><a class='item-gallery lightview' style='width: 30px;' href='images/plan/"+ photo +"' title='"+ photo +"'><img src='images/plan/"+ photo +"' style='height:30px; width:30px;' /></a></td>";
						 result += "<td>"+ this['plantName'] +"</td>";

						 var descption="";
						 if(this['plantDescription']==null)
						 { descption=""; } else { descption = this['plantDescription'];}

						 result += "<td>"+ descption +"</td>";

						 result += "<td><a onClick='updatedata("+this['plantId']+")' class='btn btn-xs btn-info waves-effect waves-light btnupd' data-toggle='modal' data-target='#myModal'><i class='ico fa fa-edit'></i></a> <a onClick='deldata("+this['plantId']+")' class='btn btn-xs btn-danger waves-effect waves-light btndel' data-remodal-target='remodal'><i class='ico fa fa-trash'></i></a></td>";
          ...
          $('#matInputFile').val("");
					...
					$.ajax({
					  ...
					  var obj = $.parseJSON(data);
						   ...
						   $('#txtcid').val(obj[0].plantId);
						   $('#txttitle').val(obj[0].plantName);
						   $('#txtdesc').val(obj[0].plantDescription);
						   $('#oldimg').val(obj[0].plantImage);
						   if(obj[0].plantImage!=null)
						   {
						   $("#img").css("display","block");
						   $("#img").attr("src","images/plan/"+obj[0].plantImage);
						  }
					  }
					  });
