            ...
            <div class="main-content">
                <div class="row small-spacing">
                    <div class="col-xs-12">
					  ...
					  <div id="success" class="alert alert-success" role="alert"  style="display:none"> <strong>Done !</strong>  Fertilizer Updated Succesfully. </div>
					  <div id="delsuccess" class="alert alert-success" role="alert"  style="display:none"> <strong>Done !</strong>  Fertilizer Deleted Succesfully. </div>

                        <div class="box-content card white">
                            <h4 class="box-title">Fertilizer Details</h4>
                            <!-- /.box-title -->
							<div class="dropdown js__drop_down">
							 <button type="type" id="btnmultidelete"  class="btn btn-icon btn-icon-right btn-danger btn-xs waves-effect waves-light" style="margin-left:10px;"><i class="ico fa fa-trash"></i> DELETE</button>
                                <a href="add_fertilizer.php" class="btn btn-icon btn-icon-right btn-violet btn-xs waves-effect waves-light"><i class="ico fa fa-plus"></i> Add New</a>
                            </div>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title" id="myModalLabel">Edit Fertilizer Details</h4>
			</div>
				                ...
				                <input type="hidden" value="update" name="action"/>
                                    ...
                                    <div class="form-group">
                                        <label for="desc">Quantity</label>
                                          <input type="text" class="form-control input-sm" id="txtdesc" name="txtdesc" placeholder="Quantity">
                                    </div>
...
function fill_data(){
	...
	$.ajax({
              ...
              success: function(data) {
					...
					$.each(obj, function() {
					     result += "<tr><td><input type='checkbox'  class='checkboxtable'  name='check[]' value='"+this['fertilizerId']+"' ></td>";
						 result += "<td>"+ this['fertilizerName'] +"</td>";
						 result += "<td>"+ this['fertilizerQuantity'] +"</td>";

						 result += "<td><a onClick='updatedata("+this['fertilizerId']+")' class='btn btn-xs btn-info waves-effect waves-light btnupd' data-toggle='modal' data-target='#myModal'><i class='ico fa fa-edit'></i></a> <a onClick='deldata("+this['fertilizerId']+")' class='btn btn-xs btn-danger waves-effect waves-light btndel' data-remodal-target='remodal'><i class='ico fa fa-trash'></i></a></td>";
				...
				function updatedata(s)
				{
					...
					$.ajax({
					  ...
					  var obj = $.parseJSON(data);
						   ...
						   $('#txtcid').val(obj[0].fertilizerId);
						   $('#txttitle').val(obj[0].fertilizerName);
						   $('#txtdesc').val(obj[0].fertilizerQuantity);
					  }
					  });
...
$(document).on('click','.confrm',function(){
     ...
     var del_id=$("#memid").val();
		...
		$(document).ready(function() {
            $("#lnkfertilizer").addClass("current");
